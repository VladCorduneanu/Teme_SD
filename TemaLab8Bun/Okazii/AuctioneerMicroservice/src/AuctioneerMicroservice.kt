...
class AuctioneerMicroservice {
    ...
    companion object Constants {
        ...
        const val AUCTION_DURATION: Long = 20_000 // licitatia dureaza 15 secunde
    ...
    init {
        ...
        taskManagerSocket = Socket(MESSAGE_PROCESSOR_HOST, TASK_PORT)
        taskManagerSocket.getOutputStream().write(Message.create("${taskManagerSocket.localSocketAddress}","am pornit auctioneer",timestamp.toString()).serialize())
...
//                val timestamp = LocalDateTime.now()
//                taskManagerSocket = Socket(MESSAGE_PROCESSOR_HOST, TASK_PORT)
//                taskManagerSocket.getOutputStream().write(Message.create("${auctioneerSocket.localSocketAddress}","Mesaj Primit de la :${message.nume}",timestamp.toString()).serialize())
//                taskManagerSocket.close()

                transmitToTaskManager("Mesaj Primit de la :${message.nume}")
            },
            onComplete = {
    ...
    private fun finishAuction() {
        ...
        try {
            ...
            transmitToTaskManager("Auctioneer-M-am oprit")
